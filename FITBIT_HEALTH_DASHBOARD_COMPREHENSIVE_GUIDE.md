...
$$	ext{Active Day (Today): } E_{	ext{total}}(t) = 	ext{BMR}_{	ext{baseline}} 	imes left( rac{H_{	ext{current}} + rac{M_{	ext{current}}}{60}}{24} ight) + E_{	ext{active}}(t)$$
...
* **Empirical Benchmark**: At 10:00 AM with 65 active calories burned:
  $$	ext{Accrued BMR} = 1,400 	imes rac{10}{24} = 583.3	ext{ kcal} implies E_{	ext{total}} = 583 + 65 = 648	ext{ kcal}$$
...
$$T_{	ext{restorative}} = T_{	ext{in_bed}} - T_{	ext{awake}}$$
...
* $S_{	ext{duration}} = minleft(50, rac{T_{	ext{restorative}}}{480} 	imes 50ight)$
* $S_{	ext{restfulness}} = maxleft(0, 30 - rac{T_{	ext{awake}}}{T_{	ext{in_bed}}} 	imes 100 	imes 1.5ight)$
* $S_{	ext{stages}} = minleft(20, rac{T_{	ext{deep}} + T_{	ext{rem}}}{T_{	ext{restorative}}} 	imes 50ight)$
...
* $	ext{Score}_{	ext{RHR}} = maxleft(0, 100 - (	ext{RHR}_{	ext{today}} - 	ext{RHR}_{14	ext{d}}) 	imes 8ight)$
...
| :--- | :--- | :--- | :--- | :--- |
| **Time-Prorated BMR Engine** | Raw fitness endpoints award full 24h BMR (~1,400 kcal) upfront at midnight, causing deceptive 70%+ completion at 8:00 AM. | Calculates BMR dynamically as $	ext{BMR}_{	ext{baseline}} 	imes rac{	ext{Hour} + 	ext{Minute}/60}{24}$ for active day; preserves full 24h baseline for past days. | Honest, circadian-aligned caloric pacing; restores athletic trust. | `HealthConnectionRepository.dart` & `DashboardScreen.dart` |
| **Net Restorative Sleep Subtraction** | APIs record total in-bed duration (562m) as sleep, ignoring restless tossing (37m), artificially inflating sleep hours. | Enforces $T_{	ext{restorative}} = T_{	ext{in_bed}} - T_{	ext{awake}}$, subtracting awake minutes: $562	ext{m} - 37	ext{m} = mathbf{525	ext{m} = 8	ext{h } 45	ext{m}}$. | Accurate restorative sleep measurement reflecting true physical recovery. | `GoogleHealthSleepData.fromJson` & `SleepRecord.dart` |
